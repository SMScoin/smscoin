...
- [Building SMScoin](#building-moondex-core)
...
git clone https://github.com/moondexcoin/moondexcoin
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/moondex/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/moondexcoin/moondexcoin
...
For example:
```bash
URL=https://github.com/crowning-/moondex.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit moondex=${COMMIT} --url moondex=${URL} ../moondex/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit moondex=${COMMIT} --url moondex=${URL} ../moondex/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit moondex=${COMMIT} --url moondex=${URL} ../moondex/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the moondex git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../moondex/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/moondexcoin.git
SIGPATH=/some/root/path/moondexcoin-detached-sigs.git

./bin/gbuild --url moondex=${BTCPATH},signature=${SIGPATH} ../moondex/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/moondex-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/moondex-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/moondex-osx-build.assert
```
...
[moondex/gitian.sigs](https://github.com/moondexcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
